...
# Original code: https://github.com/chrisroadmap/climateforcing/
...
# 2) Fix the manner of estimating overcast albedo sensitivity to cloud and non-cloud gamma and mu
...
def aprp(  # pylint: disable=too-many-arguments,too-many-locals,too-many-statements  # noqa: E501
    ...
    Aoc1 = _planetary_albedo(mu_oc_base, gamma_oc_base, alpha_oc_base)
    Aoc2 = _planetary_albedo(mu_oc_pert, gamma_oc_pert, alpha_oc_pert)
    Acs1 = _planetary_albedo(mu_cs_base, gamma_cs_base, alpha_cs_base)
    Acs2 = _planetary_albedo(mu_cs_pert, gamma_cs_pert, alpha_cs_pert)

    daoc_dacld_fwd = _planetary_albedo(mu_oc_base, gamma_oc_base, alpha_oc_pert) - Aoc1
    daoc_dacld_bwd = Aoc2 - _planetary_albedo(mu_oc_pert, gamma_oc_pert, alpha_oc_base)
    
    dacs_daclr_fwd = _planetary_albedo(mu_cs_base, gamma_cs_base, alpha_cs_pert) - Acs1
    dacs_daclr_bwd = Acs2 - _planetary_albedo(mu_cs_pert, gamma_cs_pert, alpha_cs_base)

    dacs_dmaer_fwd = _planetary_albedo(mu_cs_pert, gamma_cs_base, alpha_cs_base) - Acs1
    dacs_dmaer_bwd = Acs2 - _planetary_albedo(mu_cs_base, gamma_cs_pert, alpha_cs_pert)
    ...
    daoc_dmcld_fwd = _planetary_albedo(new_mu_oc_base, gamma_oc_base, alpha_oc_base) - Aoc1
    daoc_dmcld_bwd = Aoc2 - _planetary_albedo(new_mu_oc_pert, gamma_oc_pert, alpha_oc_pert)       
    daoc_dmaer_fwd = _planetary_albedo(new_mu_oc_pert, gamma_oc_base, alpha_oc_base) - Aoc1
    daoc_dmaer_bwd = Aoc2 - _planetary_albedo(new_mu_oc_base, gamma_oc_pert, alpha_oc_pert)
        
    # Need isolated effect of gamma_cs on gamma_oc holding gamma_cloud fixed, and vice versa:
    new_gamma_oc_pert = 1-(1-gamma_cs_pert)*(1-gamma_base) # Eq. 13
    new_gamma_oc_base = 1-(1-gamma_cs_base)*(1-gamma_pert) # Eq. 13    
    daoc_dgcld_fwd = _planetary_albedo(mu_oc_base, new_gamma_oc_base, alpha_oc_base) - Aoc1
    daoc_dgcld_bwd = Aoc2 - _planetary_albedo(mu_oc_pert, new_gamma_oc_pert, alpha_oc_pert)
    daoc_dgaer_fwd = _planetary_albedo(mu_oc_base, new_gamma_oc_pert, alpha_oc_base) - Aoc1
    daoc_dgaer_bwd = Aoc2 - _planetary_albedo(mu_oc_pert, new_gamma_oc_base, alpha_oc_pert)
